const clickToEnlarge = "单击并按住可放大。按住Shift+滚轮进行缩放。Esc重置。"
const clickToCollapse = "单击并按住可折叠。Shift+滚轮进行缩放。Esc重置。"
...
if (window.self !== window.top) {
  ...
  elements.forEach((x) => {
    document.querySelectorAll<HTMLElement>(x).forEach((div) => {
...
const addNavigationToDiv = (container: HTMLDivElement) => {
  ...
  const enablePointerEvents = (val: boolean) => {
    svgElement.querySelectorAll("a").forEach((el: HTMLElement) => {
  ...
  svgElement.addEventListener("wheel", (event: WheelEvent) => {
    ...
    if (event.deltaY > 0) {
      zoomLevel -= zoomLevel > 4 ? (zoomLevel > 6 ? (zoomLevel > 10 ? 0.4 : 0.3) : 0.2) : 0.1
    } else {
      zoomLevel += zoomLevel > 4 ? (zoomLevel > 6 ? (zoomLevel > 10 ? 0.4 : 0.3) : 0.2) : 0.1
    }
    applyTransform()
...
const processSvg = (svgContainer: HTMLDivElement) => {
...
const addImgMutationObserver = () => {
  ...
  const handleImgAddition = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      if (mutation.type === "childList") {
        mutation.addedNodes.forEach((node) => {
          if (node instanceof Element && node.querySelector(`div.excalidraw-svg`)) {
            processSvg(node.querySelector(`div.excalidraw-svg`)!)
          }
        })
      }
    }
