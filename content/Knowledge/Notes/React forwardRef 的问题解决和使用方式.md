...
- 组件库或设计系统封装
- 父组件需要直接拿到内部 DOM
- 需要和 [[React useImperativeHandle 的问题解决和核心用法|useImperativeHandle]] 一起暴露命令式 API
...
如果只是传数据或事件，优先回到 [[从渐进增强角度理解 react 组件通信方式]] 里的 `props` 和回调。
...
- 在 [[从渐进增强角度理解 react 组件通信方式]] 里，它对应“`ref` 暴露实例能力”那条路线
