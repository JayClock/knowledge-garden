...
- [[React-Router|React Router]]：从路由视角理解懒加载、数据加载与布局组织。
- [[说说 React Router 路由历史的几种模式，并说说实现原理。|React Router 的几种路由模式]]：先补齐路由基础，再看 Suspense 的加载边界。
- [[微前端的渐进式集成]]：从组件级渐进加载走向应用级渐进集成。
- [[渐进式集成：从浏览器渲染到框架设计的统一哲学]]：在更宏观的前端演进中理解 Suspense。

## 拆分卡片

- [[Suspense 的 fallback 的表达含义]]
- [[Suspense 的挂起和恢复流程的定义]]
- [[嵌套 Suspense 实现内容逐步揭示的方法]]
- [[Suspense 需要错误边界配合的原因]]
