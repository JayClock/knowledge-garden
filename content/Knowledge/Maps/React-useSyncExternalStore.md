...
- [[React useState 的问题解决和核心用法|useState]] 管组件局部状态
...
这通常是 [[React 状态管理]] 已经进入“外部 store”阶段的标志。
...
状态管理方案本身仍然要看 [[React Zustand 的核心作用和使用场景|zustand]]、[[React Jotai 的核心作用和使用场景|jotai]]、[[React Redux 的核心作用和使用场景|redux]] 或你的自定义设计。

 和并发渲染的关系

这也是为什么它会和 [[React Fiber]] 联系起来。
...
- 在 [[React Hooks]] 里，它对应“接入外部状态系统”这一层
- 在 [[React 状态管理]] 里，它是从 `Context` 走向外部 store 的关键接口
- 在 [[React Fiber]] 里，可以理解它为什么和并发渲染语义相关

## 拆分卡片

- [[React useSyncExternalStore 的问题解决]]
- [[useSyncExternalStore 中 subscribe 的职责]]
- [[useSyncExternalStore 中 getSnapshot 必须返回稳定快照的原因]]
- [[useSyncExternalStore 的适用场景]]
