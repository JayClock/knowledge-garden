...
`useContext` 是 React 组件通信从局部传值走向跨层共享时的一次增强。当前面的 `props` 和回调函数开始变成机械转发时，就说明你可能该从 [[从渐进增强角度理解 react 组件通信方式]] 进入 `Context` 了。
...
这时通常应该继续进入 [[React 状态管理]]，再看 [[React Zustand 的核心作用和使用场景|zustand]]、[[React Jotai 的核心作用和使用场景|jotai]] 或 [[React Redux 的核心作用和使用场景|redux]]。
...
- 在 [[从渐进增强角度理解 react 组件通信方式]] 里，它是从 `props` 升级到跨层共享的关键一步
- 在 [[React Hooks]] 里，它是函数组件接入共享能力的入口
- 在 [[React 状态管理]] 里，它是进入外部 store 之前的一层过渡