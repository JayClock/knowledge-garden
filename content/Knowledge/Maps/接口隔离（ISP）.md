...
接口隔离和[[单一职责（SRP）的理解|单一职责]]的区别在于，[[单一职责（SRP）的理解|单一职责]]针对的是模块、类、接口的设计。而接口隔离原则相对于单一职责原则，更加侧重于接口的设计。它提供了一种评判接口是否单一的标准：通过调用者如何使用接口来间接地判定。**如果调用者只使用部分接口或接口的部分功能，那接口的设计就不够职责单一**
 OOP 中的接口概念

## 拆分卡片

- [[接口隔离原则的核心含义]]
- [[API 集合过大时要按调用者权限和用途拆分的原因]]
- [[单个函数体现接口隔离原则的方法]]
- [[接口隔离原则和单一职责原则的区别]]
