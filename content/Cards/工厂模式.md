...
为了让类更符合[[./设计原则：单一职责（SRP）|设计原则：单一职责（SRP）]]，我们可以把 createParser（）函数剥离到一个独立的类中，让这个类只负责对象的创建。具体的代码如下所示：
```ts
...
这里的业务场景下，我们要添加 parser，肯定会改到 `RuleConfigParserFactory`的代码，在简单工厂模式的代码中，有多处 if 分支判断逻辑，违背[[./设计原则：开闭原则（OCP）|设计原则：开闭原则（OCP）]]，但权衡拓展性和可读性，这样的代码实现在大多数情况下（比如，不需要频繁地添加 parser，也没有太多的 parser）是没有问题的。
...
当我们需要添加新的规则配置解析器的时候，我们只需要创建新的 parser 并使用 factory 注册。代码改动非常少，基本上符合[[./设计原则：开闭原则（OCP）|设计原则：开闭原则（OCP）]]
# 借助工厂模式实现 简易  DI 容器
