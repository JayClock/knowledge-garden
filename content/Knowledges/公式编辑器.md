...
- 实时函数信息提示功能是如何设计的?如何保证提示信息的准确性和性能?
	**核心答案关键词**: 异步数据加载、缓存机制、动态提示、函数语法分析、性能优化(debounce)、  TypeScript 类型定义
...
```
在这里，我们发现，两个判断逻辑，除了函数名本身以外，并没有任何区别，如果要抽象，我们所有的函数计算步骤都可以划分为
1. 解析函数名称
	- 函数名称只能为大写字母组合
	- 函数名称后面只能跟 '('
2. 解析参数列表
...
3. 转义参数列表
4. 获取函数名称对应的计算逻辑
在这里我们会发现，在 [[../Cards/TDD|TDD]] 的开发模式下，我们即使一开始并不知道的**词法分析**和**语法分析**的概念，但是**随着任务功能的划分，会天然的引导我们实现，往词法分析和语法分析。
