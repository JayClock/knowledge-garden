...
接口隔离和[[../Cards/设计原则：单一职责（SRP）|单一职责]]的区别在于，[[../Cards/设计原则：单一职责（SRP）|单一职责]]针对的是模块、类、接口的设计。而接口隔离原则相对于单一职责原则，更加侧重于接口的设计。它提供了一种评判接口是否单一的标准：通过调用者如何使用接口来间接地判定。**如果调用者只使用部分接口或接口的部分功能，那接口的设计就不够职责单一**
 OOP 中的接口概念

## 拆分卡片

- [[../Cards/接口隔离原则要求调用者不依赖不需要的接口|接口隔离原则要求调用者不依赖不需要的接口]]
- [[../Cards/API 集合过大时应按调用者权限和用途拆分|API 集合过大时应按调用者权限和用途拆分]]
- [[../Cards/单个函数也应遵守接口隔离原则|单个函数也应遵守接口隔离原则]]
- [[../Cards/接口隔离原则比单一职责更关注调用者视角|接口隔离原则比单一职责更关注调用者视角]]
