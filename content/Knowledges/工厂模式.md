...
为了让类更符合[[../Cards/如何理解单一职责（SRP）？|如何理解单一职责（SRP）？]]，我们可以把 createParser（）函数剥离到一个独立的类中，让这个类只负责对象的创建。具体的代码如下所示：
```ts
...
如果 parser 可以复用，可以借助[[../Cards/单例模式的核心思想和适用场景是什么？|单例模式的核心思想和适用场景是什么？]]的特点，减少不必要的对象创建和内存消耗，代码如下：
...
这里的业务场景下，我们要添加 parser，肯定会改到 `RuleConfigParserFactory`的代码，在简单工厂模式的代码中，有多处 if 分支判断逻辑，违背[[../Cards/如何理解开闭原则（OCP）？|如何理解开闭原则（OCP）？]]，但权衡拓展性和可读性，这样的代码实现在大多数情况下（比如，不需要频繁地添加 parser，也没有太多的 parser）是没有问题的。

 工厂方法
如果我们非要把 switch 分支逻辑去掉，那该怎么办呢？很简单，利用[[../Cards/多态（Polymorphism）的定义和意义是什么？|多态（Polymorphism）的定义和意义是什么？]]的实现思路，对上面的代码进行重构。
...
当我们需要添加新的规则配置解析器的时候，我们只需要创建新的 parser 并使用 factory 注册。代码改动非常少，基本上符合[[../Cards/如何理解开闭原则（OCP）？|如何理解开闭原则（OCP）？]]
# 借助工厂模式实现 简易 DI 容器
...
- [[../Cards/简单工厂的核心思想是什么？|简单工厂的核心思想是什么？]]
- [[../Cards/简单工厂适合什么场景？|简单工厂适合什么场景？]]
- [[../Cards/工厂方法如何用多态消除集中式条件分支？|工厂方法如何用多态消除集中式条件分支？]]
- [[../Cards/为什么工厂模式可以作为简易 DI 容器的基础？|为什么工厂模式可以作为简易 DI 容器的基础？]]
