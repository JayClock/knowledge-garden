...
- 如何处理埋点数据的上报问题?是如何优化数据上报的性能和可靠性?
	...
	- **批量发送 (Batch Sending)**
		...
		- **触发上报**：SDK 会在满足以下任一条件时，将队列中的所有数据打包成一个请求（数组形式）
			...
			- **页面卸载时**：监听 `beforeunload` 事件，在页面即将关闭前，清空队列并发送队列中的所有剩余数据。
	- **缓存策略 (Caching Strategy)**
		- **上传失败检测**：当一次上报请求失败后（例如，网络错误、服务器 5xx 错误），SDK 暂存要发送的数据。
		- **数据暂存**：将发送失败的数据包，存入 `localStorage` 中，构造一个单独的"失败重试队列"。
	- **网络恢复机制 (Network Recovery Mechanism)**
		- **网络状态监听**：SDK 会通过 `navigator.onLine` 监听感知当前的网络状态。
	...
	**核心答案关键词**: 插件化架构、SDK 集成、API 设计、与后端数据接口对接、文档化接口说明
	- **SDK 集成**：
		...
		- 在应用启动时，main 函数中调用 init 方法，主要包括
			...
			- 传入可供选择的插件
	- **实施核心埋点**：
		- 当用户登录成功后，立即设置用户 ID，以便将匿名行为和登录后行为关联起来。
			```
			function onLoginSuccess(userInfo) {
			  tracker.login(userInfo.userId); // 设置用户ID
			  tracker.setProfile({ // 可以选择性设置用户属性
			    name: userInfo.name,
			    vipLevel: 'gold'
			  });
			}
			```
	- **自动采集**：如果 SDK 的 `autoTrack` 已开启 `pageview`，它通常会监听路由变化（`hashchange` 或 History API）并自动上报。
	- 
