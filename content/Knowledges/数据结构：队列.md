...
# 操作特性
和[[../Cards/数据结构：栈|数据结构：栈]]一样，也是“操作受限”的线性表，只允许在一端插入和删除数据。相比于[[../Cards/数据结构：数组|数据结构：数组]]和[[../Cards/数据结构：链表|数据结构：链表]]，更少的操作结构，意味着更加可控，不易出错的状态。
# 顺序队列：用数组实现的队列
...
- [[../Cards/队列是一种先进先出的线性数据结构|队列是一种先进先出的线性数据结构]]
- [[../Cards/顺序队列用数组实现但可能产生数据搬移|顺序队列用数组实现但可能产生数据搬移]]
- [[../Cards/链式队列适合无界排队但可能拉长等待时间|链式队列适合无界排队但可能拉长等待时间]]
- [[../Cards/循环队列通过环形下标避免顺序队列的数据搬移|循环队列通过环形下标避免顺序队列的数据搬移]]
- [[../Cards/阻塞队列用于协调生产者和消费者速度差|阻塞队列用于协调生产者和消费者速度差]]
